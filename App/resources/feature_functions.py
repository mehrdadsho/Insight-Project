import nltk
nltk.data.path.append(r"/Users/mehrdadshokrabadi/Documents/Miscellaneous/DS/Insight/StreamlitApp on AWS/NLTKdata")
...
DIRNAME = "/Users/mehrdadshokrabadi/Documents/Miscellaneous/DS/Insight/Scratch Notebooks/resources"
os.chdir(DIRNAME)
retval = os.getcwd()
...
#             text=''.join(t).strip()
    ...
    def load_happiness_index_lexicon(self, filepath=DIRNAME):
        word_to_happiness = {}
    ...
    def happiness_index_feats(self, text):
        ...
        with  open(os.path.join(DIRNAME, "stopwords.txt")) as stopdata:
    ...
    def load_moral_foundations_lexicon(self, filepath=DIRNAME):
        code_to_foundation = {}
        foundation_to_lex = {}
    ...
    def load_acl13_lexicons(self, filepath=DIRNAME):
        ...
        with open(os.path.join(filepath, "negative-words.txt"),encoding = "ISO-8859-1") as lex:
    ...
    def POS_features(self, text):
        ...
        wordandtag = tags;#line.strip().split()
        tags = [wordandtag[i][1] for i in range(len(wordandtag))]
#         ccc
#         try:
#             tags = [wordandtag[i][1] for i in range(len(wordandtag))]
#         except:
...
#         return counts_norm
    ...
    def wordlen_and_stop(self, text):
        with open(os.path.join(DIRNAME,"stopwords.txt")) as data:
...
#         loaded_model = pd.read_spss(open(os.path.join(DIRNAME, '', 'NB_Subj_Model.sav'), 'rb'))
...
#         result = loaded_model.predict_proba(X_new_tfidf)
        prob_obj = 0;#result[0][0]
        prob_subj = 0;# result[0][1]
        return prob_obj, prob_subj


    def load_LIWC_dictionaries(self,
                           filepath=DIRNAME):
        cat_dict = {}
        stem_dict = {}
        counts_dict = {}
